...
export default function CompanyJobs() {
  const [jobs, setJobs] = useState<Job[]>([]); // Specify type
  const router = useRouter();

  useEffect(() => {
    fetch("/api/company/jobs")
      .then((res) => res.json())
      .then((data: Job[]) => setJobs(data)) // Ensure TypeScript knows the expected type
  ...
  async function handleDelete(jobId: string) {
    ...
    if (response.ok) {
      setJobs((prevJobs) => prevJobs.filter((job) => job.id !== jobId)); // Update jobs safely
    } else {
      alert("Failed to delete job.");
    }
  }
